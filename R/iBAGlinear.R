...
iBAG <- function(gene_expr,cnv,meth,out,iBAGtype="linear",fast=T,verbose=F){
  ...
  if(fast){
    res = EMVS::EMVS(to_gibbs$Y,to_gibbs$X, 0.001, 1000)
    ...
    result$post_means <- apply(final$PARAM[(burn_in+1):nrow(final$PARAM),],2,mean)
...
# for the clinical model (2nd layer model)
# data prep clinical data & the results from 1st model
