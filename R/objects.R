...
iBAG_results <- R6Class("iBAG_results",
  public = list(
    ...
    #' @param SS (list(NULL,NULL)): a list of outcomes representing the Sum of Squares Error from the mechanistic model.
    #' @param beta_mean (NULL): a vector of posterior means for the coefficients from the clinical model.
    #' @param beta_incl_prob (NULL): a vector of posterior inclusion probabilities for the coefficients from the clinical model.
    ...
    #'    - n_patients is derived as the length(Y)
    #'    - check that ncol(X)%n_data == 0
    #'      - if not throw error
    #'      - if true: n_genes = ncol(X)%/%n_data
    #'    - check that nrow(X) == n_patients
    #'      - if not throw error
    #'    - for each item in SS
    #'      - check that dim(item) == (n_patients,n_genes)
    #'        - if not throw error
