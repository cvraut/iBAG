...
empty.X.constructor <- function(patients,
  ...
  colnames(X) <- paste(rep(sapply(1:k,FUN = X.namer),each=p),rep(genes,k),sep="_")
  return(X)
}
...
mech.model <- function(mrna,
  ...
  X <- empty.X.constructor(row.names(mrna), genes, k, names(data.list), other.name, sep, default.data.name, DEBUG)
  SS <- matrix(NA,nrow=p,ncol = k+1)
  colnames(SS) <- c("SST",sapply(1:(k-1),FUN = function(i){sprintf("SSD%d",i)}),"SSO")
  row.names(SS) <- genes
